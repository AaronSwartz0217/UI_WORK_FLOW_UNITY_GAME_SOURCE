...
所有普通文字均由 Unity TextMeshPro 渲染。登录、角色选择和主界面现已组成同一测试流程；主界面玻璃与前两页的中心透明度同步为约 64/255。运行时 Shader 保留 PNG 源 Alpha，并继续乘以 uGUI `Image.color.a`；主界面另设显式最大不透明度，避免玻璃区域在场景中重新显得实心。
...
- 主界面运行时最大不透明度：大面板/地图 0.13，道具栏 0.14，按钮 0.16
